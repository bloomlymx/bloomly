...
export default async function ShopPage() {
  ...
  // 2. Buscamos FLORES EN RIESGO para generar "Ofertas Relámpago" ⚡
  const riskBatches = await prisma.flowerBatch.findMany({
    where: { status: 'RISK', quantity: { gt: 0 } }
  });

  // 3. Obtenemos la fecha actual para validar promociones
  const now = new Date();

  // 4. NUEVO: Traer configuración de la tienda 🏪
  // Si no se ha configurado nada, usamos "Florería Demo" por defecto
  ...
  const categories = [
    { name: "San Valentín", image: "https://static.wixstatic.com/media/e50e1f_7a38499be5c541d281e8aed4f99e467c~mv2.png/v1/fill/w_556,h_834,al_c,q_90,usm_0.66_1.00_0.01,enc_avif,quality_auto/e50e1f_7a38499be5c541d281e8aed4f99e467c~mv2.png" },
    { name: "Cumpleaños", image: "https://i0.wp.com/floresyregalosmx.com/wp-content/uploads/Captura-de-pantalla-2024-12-11-a-las-12.09.40%E2%80%AFa.m.png?fit=1094%2C1432&ssl=1" },
    { name: "Aniversario", image: "https://www.floreriabloom.com/cdn/shop/files/BUCHON_100ROSAS.jpg?v=1748289881&width=1200" },
    { name: "Condolencias", image: "https://lavioletera.com.mx/cdn/shop/products/condolenciasarreglo2_600x.jpg?v=1654558723" },
  ];
  ...
  return (
    <main className="min-h-screen bg-gray-50 font-sans text-gray-900">
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 cursor-pointer">
            <div className="w-8 h-8 bg-bloomly-pink rounded-full flex items-center justify-center text-white font-bold text-lg">
                {shopName.charAt(0)} {/* Primera letra del nombre */}
            </div>
            <span className="text-xl font-bold text-gray-800 tracking-tight">{shopName}</span>
          </div>

          <div className="flex items-center gap-6 text-sm font-medium text-gray-500">
            <Link href="#" className="hidden md:block hover:text-bloomly-olive transition-colors">Catálogo</Link>
      ...
      {/* OFERTAS RELÁMPAGO (Riesgo) */}
      {riskBatches.length > 0 && (
        <div className="bg-red-50 border-b border-red-100 py-12">
            <div className="max-w-7xl mx-auto px-4">
                <div className="flex items-center gap-3 mb-6 animate-pulse">
                    <span className="text-3xl">⚡</span>
                    <h2 className="text-2xl font-bold text-red-600 tracking-tight">Ofertas Relámpago</h2>
                    <span className="bg-red-200 text-red-700 text-[10px] font-bold px-2 py-1 rounded uppercase">
                        Solo por hoy
                    </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {riskBatches.map((batch) => (
                        <div key={batch.id} className="bg-white border border-red-100 rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
                            <div className="absolute top-0 right-0 bg-red-500 text-white text-[10px] font-bold px-3 py-1 rounded-bl-lg z-10">
                                -30% OFF
                            </div>
                            <div className="w-full h-40 bg-gray-100 rounded-xl mb-4 flex items-center justify-center text-4xl group-hover:scale-105 transition-transform">
                                🥀
                            </div>
                            <h3 className="font-bold text-gray-800 text-lg leading-tight mb-1">Promo: {batch.flowerType}</h3>
                            <div className="flex justify-between items-end">
                                <div>
                                    <span className="text-xs text-gray-400 line-through mr-2">$500</span>
                                    <span className="text-xl font-bold text-red-500">$350</span>
                                </div>
                                <button className="bg-red-500 text-white w-8 h-8 rounded-full flex items-center justify-center hover:bg-red-600 transition-colors shadow-sm">+</button>
                            </div>
                        </div>
                    ))}
                </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-16">
        <div className="flex justify-between items-end mb-8">
            ...
            <Link href="#" className="text-xs font-bold text-bloomly-olive border-b border-bloomly-olive pb-0.5 hover:opacity-80">VER TODO</Link>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {categories.map((cat, idx) => (
            <div key={idx} className="group relative h-60 rounded-xl overflow-hidden cursor-pointer shadow-md">
      ...
      {/* 4. PRODUCTOS (Grid E-commerce) con Lógica de Precios */}
      <div className="bg-white py-16 border-t border-gray-100">
        <div className="max-w-7xl mx-auto px-4">
            <h2 className="text-3xl font-bold text-gray-900 mb-2 text-center">Favoritos de la Semana 🌟</h2>
            ...
            {products.length > 0 ? (
                ...
                {products.map((product) => {
                    ...
                    return (
                        ...
                        <div className="relative aspect-[4/5] overflow-hidden rounded-2xl bg-gray-100 mb-4 shadow-sm group-hover:shadow-xl transition-all duration-300">
                            ...
                            {/* Badge Dinámico: Oferta vs Nuevo */}
                ...
                <div className="text-center py-20 bg-gray-50 rounded-3xl border border-dashed border-gray-200">
                    <p className="text-xl text-gray-400 font-medium">Aún no hay productos en el catálogo.</p>
                </div>
            )}
        </div>
      </div>
